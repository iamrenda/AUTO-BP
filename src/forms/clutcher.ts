...
export const clutcherForm = async function (player: Player): Promise<ActionFormResponse> {
  const form = new ChestFormData("27")
    ...
    .pattern(["_________", "_r_s_b_q_", "_________"], {
      r: {
        itemName: "§aStart",
        itemDesc: [],
  ...
  const form = new ChestFormData("27")
    ...
    .pattern(["_________", "_s_______", "_________"], {
      s: {
        ...
        itemDesc: ["", clutcherTs.tempData["clutchShiftStart"] ? "§aEnabled" : "§cDisabled"],
        texture: clutcherTs.tempData["clutchShiftStart"]
...
export const clutchNumForm = async function (player: Player): Promise<ActionFormResponse> {
  ...
  for (let i = 0; i <= 8; i++)
    form.button(i + 9, `${i + 1} Hits`, [], "minecraft:blaze_rod", i + 1, false);
  return form.show(player);
};
